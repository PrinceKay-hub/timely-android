...
class ReBook extends StatefulWidget {
  ...
  const ReBook({super.key, required this.data});
...
class _ReBookState extends State<ReBook> {
  ...
  bool get _isSelectedDateWorkingDay {
   ...
   // user notification for pending
  void sendnotification( String userIDs,) async {
    final snapshot =
    ...
    return BlocConsumer<BookingCubit, BookingState>(
      listener: (context, state) {
        if (state is BookingSuccess) {
          ...
          sendnotification(widget.data['providerId']);
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '${service['duration'].toString()} mins',
                        style: const TextStyle(
                          color: Colors.grey,
                          fontSize: 12,
                        ),
  ...
  void _showBookingSummary(BuildContext context) {
    List<Map<String, dynamic>> services = List<Map<String, dynamic>>.from(widget.data['services']);
    final cubit = context.read<BookingCubit>();
    final selectedService = widget.data['services'][_selectedServiceIndex];
    ...
    if (_selectedServiceIndex! < 0 ||
        _selectedServiceIndex! >= widget.data['services'].length) {
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    ...
                    // Prepare and submit booking data
                    final booking = BookingEntity(
                      id: '', // Create a method for this
                      ...
                      status: 'pending',
                      latitude: widget.data['latitude'] ?? 0.0,
